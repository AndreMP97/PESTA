        ...
        if ($auth == "1"):
            ...
            if ($usertype == "diretor"):
                ...
                // IF THERE ARE NO EMPTY FIELDS THEN-
                    ...
                    try {
                        $fetch_internships = "SELECT propostas.id_proposta, propostas.title, atribuicoes.orientador, atribuicoes.data_fim, atribuicoes.numero_aluno
                        FROM `propostas`
                        ...
                        AND atribuicoes.data_fim IS NOT NULL
                        AND atribuicoes.numero_aluno IS NOT NULL
                        AND `concluida` = '1'
                        AND `aprovada` = '1'
                        ORDER BY atribuicoes.data_fim";
                        ...
                        if($search_stmt->rowCount()):
                            while($row = $search_stmt->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                $concluded_date[] = date("d-m-Y", strtotime($row['data_fim']));
