...
import Assign from '../diretor/AssignSupervisor';
...
class AssignSupervisor extends Component {
    ...
    render() {
        ...
        if (localStorage.getItem("sessionID") && this.state.loading === false && this.state.auth && this.state.error === false) {
            if (this.state.data.usertype === "diretor" && (sessionStorage.getItem("typeProposal") === "internship" || sessionStorage.getItem("typeProposal") === "project")) {
                return (
                    <Assign />
                );
            }
