...
class ListFeedbackConcluded extends Component {
    _isMounted = false;
    ...
    async componentDidMount() {
        ...
        try {
            ...
            await axios.get(window.website.concat("docentes/pareceresconcluidos.php"))
                .then(request => {
    ...
    populateProposals() {
        ...
        const id_feedback = Object.values(this.state.data.id_parecer);
        ...
        head.push(
            <tr key = {0}>
                ...
                <th><span style={{fontSize: 'small', fontFamily: 'verdana, geneva', color: '#FFFFFF'}}>Concluido em </span> </th>
                <th><span style={{fontSize: 'small', fontFamily: 'verdana, geneva', color: '#FFFFFF'}}>Parecer </span> </th>
            </tr>
        );
        
        for (var i = min; i < max; i++) {
            const pid_feedback = id_feedback[i];
            body.push(
                <tr key={i}>
                    <td>
                    ...
                    <td>
                        <Link to='/darparecer' onClick={() => {sessionStorage.setItem("id_feedback", pid_feedback); sessionStorage.setItem("feedback", "view"); sessionStorage.setItem("currentPage", this.state.currentPage);}}> 
...
export default ListFeedbackConcluded;